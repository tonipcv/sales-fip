...
function SucessoMembrosAutomacaoContent() {
  ...
  const referralLink = `${baseUrl}/membros-teste-automacao/cadastro?indicacao=${encodeURIComponent(referralCode)}`;
  ...
  const whatsappHref = useMemo(() => {
    const msg = `Olá! Fiz meu cadastro na Automação. Meu cupom: ${coupon}. Pode me ajudar?`;
    return `https://wa.me/?text=${encodeURIComponent(msg)}`; // (x) placeholder generic WhatsApp share
  }, [coupon]);
...
export default function SucessoMembrosAutomacao() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-black" />}> 
      <SucessoMembrosAutomacaoContent />
    </Suspense>
  );
}
